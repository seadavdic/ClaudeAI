...
│         ▼                                                               │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │     🖥️  k3s-worker-1 (192.168.178.101)                          │   │
│  │          Raspbian GNU/Linux 10 (ARMv7)                          │   │
│  │                                                                 │   │
│  │  ┌─────────────────────────────────────────────────────────┐   │   │
│  │  │              ⚙️  k3s Worker Node                         │   │   │
...
│  │  │     ├─ Port: 5000                                    │      │   │
│  │  │     ├─ Namespace: rest-api                           │      │   │
│  │  │     ├─ Ingress: http://api.local:30683               │      │   │
│  │  │     ├─ Endpoints: /health, /api/users, /api/process  │      │   │
...
└──────┬───────┘
       │
       │ 1. DNS: grafana.local → 192.168.178.100
       │    (from hosts file)
       │
       ▼
┌──────────────────────┐
│ 192.168.178.100:30683│
│ (k3s-master)         │
│                      │
│  Traefik Ingress     │
└──────┬───────────────┘
...
START
  ...
  │   ├─ Set static IPs (192.168.178.100, 192.168.178.101)
  │   ├─ Identified WiFi interface (wlan0)
  │   └─ ✅ Stable network addresses
  │
  ├─► Step 3: k3s Installation
  │   ├─ Master: k3s server on k3s-master (192.168.178.100)
  │   ├─ Worker: k3s agent joins cluster on k3s-worker-1
...
│  │  192.168.178.100   │  WiFi  │  192.168.178.101   │           │
│  │  wlan0             │        │  wlan0             │           │
│  └────────────────────┘        └────────────────────┘           │
└─────────────────────────────────────────────────────────────────┘
```
...
└─────────────────────────────────────────────────────────────────┘
  ...
  └─ ✅ Solution: Change to master IP (k3s-master: 192.168.178.100)
